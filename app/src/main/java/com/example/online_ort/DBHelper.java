...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String COLUMN_JOURNAL_CORRECT = "JOURNAL_CORRECT";
    private static final String COLUMN_JOURNAL_COUNT = "JOURNAL_COUNT";
    private static final String COLUMN_JOURNAL_COUNT_MAX = "JOURNAL_COUNT_MAX";
    private static final String COLUMN_JOURNAL_COUNT_QUESTION = "JOURNAL_COUNT_QUESTION";
    private static final String COLUMN_JOURNAL_TIME = "JOURNAL_TIME";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        db.execSQL("CREATE TABLE " + JOURNAL_TABLE + " (" +
                COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " +
                COLUMN_JOURNAL_ID + " TEXT, " +
                COLUMN_JOURNAL_CORRECT + " TEXT, " +
                COLUMN_JOURNAL_COUNT + " TEXT, " +
                COLUMN_JOURNAL_COUNT_MAX + " TEXT, " +
                COLUMN_JOURNAL_COUNT_QUESTION + " TEXT, " +
                COLUMN_JOURNAL_TIME + " TEXT);");
    }
    ...
    public void addJournalEntry(String journalId, String journalCorrect, String journalCount, String journalCountMax, String journalCountQuestion, String journalTime) {
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues cv = new ContentValues();
        cv.put(COLUMN_JOURNAL_ID, journalId);
        cv.put(COLUMN_JOURNAL_CORRECT, journalCorrect);
        cv.put(COLUMN_JOURNAL_COUNT, journalCount);
        cv.put(COLUMN_JOURNAL_COUNT_MAX, journalCountMax);
        cv.put(COLUMN_JOURNAL_COUNT_QUESTION, journalCountQuestion);
        cv.put(COLUMN_JOURNAL_TIME, journalTime);

        // Вставляем данные в таблицу журнала
    ...
    Cursor readAllDataJournal() {
        SQLiteDatabase db = this.getReadableDatabase();

        Cursor cursor = null;
        if(db != null) {
            cursor = db.rawQuery("SELECT * FROM " + JOURNAL_TABLE, null);
